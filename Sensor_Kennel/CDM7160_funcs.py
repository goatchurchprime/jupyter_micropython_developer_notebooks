...
n = 0
i2c = None
def setupCO2filters(li2c):
    global n, butterworthfilter, i2c
    i2c = li2c
    n = 0
    butterworthfilter = ABfilter(b, a)

def readCO2Filtered():  # n should iterate from 0
    global n, expsmoothfactor, validrange, co2ppmSmoothed
    ...
    if co2ppm not in badreadings:
        ...
        elif n == 20:  expsmoothfactor, validrange = 0.15, 160
        co2ppmSmoothed = co2ppm*expsmoothfactor + co2ppmSmoothed*(1.0-expsmoothfactor)
        if abs(co2ppmSmoothed - co2ppm) < validrange:
            if n < 9:
                for i in range(19):  # bootstrap filter to early readings 
                    butterworthfilter.addfiltvalue(co2ppm)
            bfiltco2ppm = butterworthfilter.addfiltvalue(co2ppm)
            n += 1
            return n, co2ppm, bfiltco2ppm
    return n, 0.0, -co2ppm
